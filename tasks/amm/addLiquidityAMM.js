...
task('addLiquidityAMM', 'addLiquidityAMM')
  ...
  .setAction(async ({ amounta, amountb, option, owner, configuration }, hre) => {
    ...
    const poolAddress = await ethers.getContractAt('OptionPoolRegistry', await registry.getPool(option))
    console.log('poolAddress', poolAddress.address)
    const pool = await ethers.getContractAt('OptionAMMPool', poolAddress.address)
